...
export default function CheckoutPage() {
  ...
  const payPix = async () => {
    try {
      const resp = await apiPost('/api/payments/pix', { amount: order.totalAmount, description: `Pedido ${id}` });
  ...
  return (
    <Card title="Checkout" extra={<span>Total: R$ {order.totalAmount.toFixed(2)}</span>}>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
        <Button type="primary" onClick={payPix} style={{ background: '#E63946' }}>Pagar com PIX</Button>
      </div>
    </Card>
  );
}
